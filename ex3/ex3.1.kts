...
fun main(args: Array<String>) {
    ...
    val intersections = wires.map { it.points }
            .reduce { min, it -> min.apply { retainAll(it) } } // intersect wires
            .filter { it != Point(0, 0) } // hack
...
fun parseWire(wire: List<String>): Wire {
    ...
    return Wire(wire.asSequence().map {
        ...
        when (it[0]) {
            'R' -> (x..(x + coord)).map { x = it; Point(it, y) }
            'L' -> (x downTo x - coord).map { x = it; Point(it, y) }
            'D' -> (y downTo y - coord).map { y = it; Point(x, it) }
            'U' -> (y..(y + coord)).map { y = it; Point(x, it) }
