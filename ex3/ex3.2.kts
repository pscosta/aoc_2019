...
data class Wire(val points: HashSet<Point>)

fun main(args: Array<String>) {
    val wires = args[0].split("\n").map { it.split(",") }.map { parseWire(it) }

    val intersections = wires.map { it.points }.reduce { min, it -> min.apply { retainAll(it) } }
    println(intersections.map { manhattanDistance(Point(0, 0), it) }.min())

    println(intersections.map { cross ->
        val p1 = wires[0].points.first { it.x == cross.x && it.y == cross.y }
        val p2 = wires[1].points.first { it.x == cross.x && it.y == cross.y }
        p1.steps + p2.steps
    }.min())
}
...
fun parseWire(wire: List<String>): Wire {
    var x = 0
    var y = 0
    var steps = 1
    val points = HashSet<Point>()

    return Wire(wire.asSequence().map {
        val coord = it.substring(1).toInt()
